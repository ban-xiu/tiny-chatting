...
import com.tinychating.common.chatai.dto.GPTRequestDTO;
import com.tinychating.common.chatai.properties.ChatGLM2Properties;
import com.tinychating.common.chatai.utils.ChatGLM2Utils;
...
public class ChatGLM2Handler extends AbstractChatAIHandler {
    ...
    protected String doChat(Message message) {
        ...
        try {
            ...
            return FrequencyControlUtil.executeWithFrequencyControl(FrequencyControlStrategyFactory.TOTAL_COUNT_WITH_IN_FIX_TIME_FREQUENCY_CONTROLLER, frequencyControlDTO, () -> sendRequestToGPT(new GPTRequestDTO(content, uid)));
    ...
    private Long userMinutesLater(Long uid) {
        // 获取用户最后聊天时间
        Date lastChatTime = RedisUtils.get(RedisKey.getKey(RedisKey.USER_GLM2_TIME_LAST, uid), Date.class);
